...
(function(){
  ...
  function renderModuleTabs(){
    ...
    if(!bar){
      console.warn('renderModuleTabs: #tabsbar not found');
      return;
    }
    console.log('renderModuleTabs running — modules count:', state.modules && state.modules.length);
    bar.innerHTML = '';
  ...
  function renderDashboard(container){
    ...
    card.className = 'card';
    card.innerHTML = '<div style="font-weight:800">Investor Dashboard</div><div class="muted">Profiles, commitments, and investor list.</div>';
  ...
  function renderFinder(container){
    var card = document.createElement('div');
    card.className = 'card';
    card.innerHTML = '<div style="font-weight:800">Investment Finder</div><div class="muted">Search and filters for deals.</div>';
    container.appendChild(card);
  }
  ...
  function render(){
    try{
      ...
      if(state.activeModule === 'dashboard') renderDashboard(mount);
      if(state.activeModule === 'finder') renderFinder(mount);
      if(state.activeModule === 'dealanalyzer') renderDealAnalyzer(mount);
      root.appendChild(mount);
      console.log('render completed; activeModule=', state.activeModule);
    }catch(err){
      console.error('Error in render()', err);
      var app = document.getElementById('app');
      if(app) app.innerHTML = '<div class="card" style="border-color: #ef4444; background:#fff6f6; color:#7f1d1d">An error occurred. See console for details.</div>';
    }
  }
