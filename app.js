...
(function(){
  ...
  function renderModuleTabs(){
    ...
    for(var i=0;i<state.modules.length;i++){
      (function(m){
        var btn = document.createElement('button'); btn.className = 'pill' + (state.activeModule===m.id? ' active':''); btn.textContent = m.label; btn.addEventListener('click', function(){ state.activeModule = m.id; localStorage.setItem(LS_ACTIVE, m.id); render(); });
        bar.appendChild(btn);
      })(state.modules[i]);
    }
    // Plan label
  ...
  function renderDashboard(root){
    ...
    for(var i=0;i<state.investors.length;i++){ var it = state.investors[i]; listHtml += '<div style="padding:8px;border:1px solid var(--border);border-radius:8px;margin-bottom:8px">' + escapeHtml(it.name) + '<div class="muted-small">' + escapeHtml(it.contact) + '</div><div class="muted-small">Committed: '+money(it.committed)+'</div></div>'; }
    listHtml += '<div style="margin-top:8px;font-weight:700">Add Investor</div><div class="field"><input id="newInvName" placeholder="Name"></div><div class="field"><input id="newInvContact" placeholder="Contact"></div><div class="field"><input id="newInvCommitted" type="number" placeholder="Committed"></div><div><button id="addInvestorBtn" class="btn btn-ghost">Add Investor</button></div>';
    invCard.innerHTML = listHtml;
    root.appendChild(invCard);

    // Bind events
    (function(){ var btn = document.getElementById('saveProfile'); if(btn){ btn.addEventListener('click', function(){ state.profile.name = document.getElementById('pName').value || ''; state.profile.email = document.getElementById('pEmail').value || ''; state.profile.commitment = Number(document.getElementById('pCommit').value)||0; localStorage.setItem(LS_PROFILE, JSON.stringify(state.profile)); alert('Profile saved'); }); }
      var addBtn = document.getElementById('addInvestorBtn'); if(addBtn){ addBtn.addEventListener('click', function(){ var n = document.getElementById('newInvName').value.trim(); if(!n) return alert('Enter a name'); var contact = document.getElementById('newInvContact').value.trim(); var c = Number(document.getElementById('newInvCommitted').value)||0; var id = Math.max(0, applyIds(state.investors))+1; state.investors.push({id:id,name:n,contact:contact,committed:c,deployed:0}); render(); }); }})();
  }
  ...
  function renderDealAnalyzer(root){
    ...
    for(var i=0;i<state.scenarios.length;i++){ var s = state.scenarios[i]; fh += '<div style="padding:8px;border:1px solid var(--border);border-radius:8px;margin-bottom:8px"><div style="font-weight:700">'+escapeHtml(s.name)+'</div><div class="muted-small">Loan: '+money(s.loanAmt)+' • Rate: '+(s.rate*100).toFixed(2)+'% • Term: '+s.termYears+' yrs</div><div style="margin-top:6px"><button class="btn-ghost applySc" data-sid="'+s.id+'">Apply</button></div></div>'; }
    ...
    for(var r=0;r<state.t12.rows.length;r++){ var row = state.t12.rows[r]; t12html += '<tr><td style="padding:6px;border-bottom:1px solid var(--border)">'+escapeHtml(row.category)+'</td><td style="padding:6px;border-bottom:1px solid var(--border);text-align:right">'+money(row.months.reduce(function(a,b){return a+(Number(b)||0);},0))+'</td></tr>'; }
    t12html += '</table>';
    ...
    // Bind events
    (function(){ var apply = document.getElementById('applyDealBtn'); if(apply){ apply.addEventListener('click', function(){ state.deal.name = document.getElementById('in_name').value || ''; state.deal.market = document.getElementById('in_market').value || ''; state.deal.price = Number(document.getElementById('in_price').value)||0; state.deal.downPct = Number(document.getElementById('in_downPct').value)||0; state.deal.downAmt = state.deal.price * state.deal.downPct; state.deal.loanAmt = state.deal.price - state.deal.downAmt; state.deal.rate = Number(document.getElementById('in_rate').value)||0; state.deal.termYears = Number(document.getElementById('in_term').value)||30; render(); }); }
      var applyBtns = document.getElementsByClassName('applySc'); for(var j=0;j<applyBtns.length;j++){ applyBtns[j].addEventListener('click', function(ev){ var sid = Number(ev.target.getAttribute('data-sid')); var sc = state.scenarios.find(function(x){ return x.id===sid; }); if(sc){ state.deal.loanAmt = sc.loanAmt; state.deal.rate = sc.rate; state.deal.termYears = sc.termYears; state.deal.downAmt = state.deal.price - state.deal.loanAmt; render(); } }); }
      var addSc = document.getElementById('addScenarioBtn'); if(addSc){ addSc.addEventListener('click', function(){ var id = Math.max(0, applyIds(state.scenarios))+1; state.scenarios.push({id:id,name:'New Scenario',loanAmt:state.deal.loanAmt,rate:state.deal.rate,termYears:state.deal.termYears}); render(); }); }
      var exp = document.getElementById('exportT12'); if(exp){ exp.addEventListener('click', function(){ var rows = [['Category','Annual']]; for(var k=0;k<state.t12.rows.length;k++){ var row = state.t12.rows[k]; var total = row.months.reduce(function(a,b){ return a + (Number(b)||0); },0); rows.push([row.category, total]); } var csv = rows.map(function(r){ return r.map(function(c){ var s = String(c); if(s.indexOf(',')!==-1) s='"'+s.replace(/"/g,'""')+'"'; return s; }).join(','); }).join('\n'); downloadFile('t12_export.csv', csv, 'text/csv'); }); }
    })();
  }

  // Finder (light)
  function renderFinder(root){ var c = document.createElement('div'); c.className='card'; c.innerHTML = '<div style="font-weight:800">Investment Finder</div><div class="muted-small" style="margin-top:8px">Search sample listings and open in Analyzer.</div><div style="margin-top:8px"><button id="openSample" class="btn">Open Sample Deal</button></div>'; root.appendChild(c); var b = document.getElementById('openSample'); if(b) b.addEventListener('click', function(){ state.deal = { name:'Sample 14-unit', market:'Orlando, FL', price:1500000, units:14, downPct:0.35, downAmt:525000, loanAmt:975000, rate:0.065, termYears:30, vacancyRate:0.06, otherIncome:0 }; state.activeModule='dealanalyzer'; localStorage.setItem(LS_ACTIVE,'dealanalyzer'); render(); }); }
